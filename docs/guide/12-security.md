...
Both modes compare in constant time (`MessageDigest.isEqual`), so the secret cannot leak via timing. The secret (`protean.worker.admin-auth.secret`) is either pinned explicitly (externally managed) or, left blank, auto-generated as a random 256-bit token once per JVM lifetime on the main and injected into each spawned worker/container at launch. **Secrets are handed over in a file, never on the command line** — see the note below. Transport confidentiality (TLS) is out of scope — the plane is assumed localhost/host-scoped. See [03. Configuration](03-configuration.md) for the full key table.

> **Secrets are never passed on a worker's command line.** The process table is world-readable, so an auth secret or a scoped DB password on argv could be read by any local user; for a container it is worse, because `docker run` arguments remain visible to `docker inspect` for the container's whole life. Protean writes them to an owner-only (`rw-------`) file and hands the worker `--spring.config.import=optional:file:…`, bind-mounting that file read-only into a container. Only a path is externally visible. Files are removed when the worker retires and purged at startup if a previous run left any. See [07. Data Access](07-data-access.md) for the scoped-DB side.
