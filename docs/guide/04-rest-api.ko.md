...
- `tickMs` — 푸시 간격(`1000`). 클라이언트의 침묵 감지 워치독이 가정 대신 계약을 기준으로 자기 값을 정할 수 있게 한다.

  ⚠️ **이 값은 하한이지 보장이 아니다.** 티커는 *fixed delay* 로 걸려 있어 다음 주기는 이전 주기가 **끝난** 뒤 이만큼 지나 시작한다 — 그리고 한 주기는 모듈을 전수 조회하고, 링 전체로 윈도를 다시 계산하고, 연결된 클라이언트마다 차례로 네 프레임을 직렬화해 보낸다. 부하가 커지면 실효 간격이 늘어난다. 워치독은 `tickMs` 의 **넉넉한 배수**로 잡아야 한다 — 정확한 주기로 믿는 클라이언트는 멀쩡한 서버를 멈춘 것으로 보고하게 된다.
