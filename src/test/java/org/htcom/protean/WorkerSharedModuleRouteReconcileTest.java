...
/**
 * A shared-module-consuming route works when the consumer is <b>worker</b>-isolated (the main pushes the
 * {@code uses} closure into the worker; the route is served through the ReverseProxy) and survives a main
 * restart (reconcile). The library stays in-process. Mirrors {@link InProcessSharedModuleRouteReconcileTest}.
 */
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
@AutoConfigureMockMvc
class WorkerSharedModuleRouteReconcileTest {

    static final Path STORE_DIR = Path.of(System.getProperty("java.io.tmpdir"), "protean-sm-route-worker");
    ...
    @Autowired WorkerProcessIsolation workerIsolation; // consumer
    ...
    @Test
    void worker_route_consumes_shared_module_and_survives_reconcile() throws Exception {
        platform.install(SharedModuleRouteScenario.library());
        platform.install(SharedModuleRouteScenario.consumer("worker"));

        mockMvc.perform(get(SharedModuleRouteScenario.ROUTE).param("x", "1").param("y", "2"))
                .andExpect(status().isOk())
                .andExpect(content().string("at=point(11,12)"));

        // Forced reboot: drop the worker deployment + the library, keep the store.
        workerIsolation.undeploy(SharedModuleRouteScenario.CONSUMER_ID);
