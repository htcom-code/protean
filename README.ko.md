...
- **재기동 복구** — 디스크립터를 write-ahead 저장소(파일시스템 또는 JDBC)에 영속화하고, 서버 재기동 시
  `reconcile` 이 ACTIVE 모듈을 자동 복원한다. 인메모리 상태에 의존하지 않는다.
...
- **모듈 데이터 접근 계약** — 엔진 무관 메커니즘(리소스 채널·멀티 DataSource·모듈당 DB 스코프 자동 프로비저닝)만
...
- **DB 스코프 자동 프로비저닝** — `protean.worker.db.auto-provision` 으로 모듈당 전용 DB/스키마+유저를 생성한다
  (MySQL/Postgres 내장, 벤더 확장은 `DbDialect` 빈).
...
| `protean.module-store.backend` / `.dir` | `filesystem` | 디스크립터 저장소: filesystem\|jdbc |
...
| `protean.worker.db.auto-provision` | `false` | 모듈당 격리 DB 스코프 자동 생성(dialect·admin 자격 필요) |
...
- 산출물: plain jar(소비용, classifier 없음) + `-boot.jar`(embed 워커 실행형 fat jar).
