...
> **활성 키 주의**: 디버그 표면은 **노출**과 **실행**이 분리돼 있다.
> - **노출**은 `protean.mcp.enabled=true` 면 자동이다 — `DebugMcpConfiguration` 의 `@ConditionalOnProperty` 는
>   (debug 키가 아니라) `protean.mcp.enabled` 에 걸리므로, MCP 가 켜지면 `debug.*` 툴은 항상 `tools/list` 에 뜬다.
> - **실행**은 별도로 **`protean.mcp.debug.enabled`(점, 기본 false, 런타임 토글 가능)** 로 켠다. 꺼져 있으면 debug
>   호출이 dispatcher 에서 `debug surface disabled` 로 거부된다. 반드시 점 표기를 써야 한다 —
>   `protean.mcp.debug-enabled`(하이픈)는 다른 키로 취급돼 바인딩되지 않아 표면이 꺼진 채(기본 false) 남는다.

먼저 `debug.*` 툴(attach·set_breakpoint·await_stop·frames·get_variables·evaluate·step·continue·redefine·terminate)이
`tools/list` 에 노출됐는지 본다(MCP 가 켜졌으면 `debug.enabled` 와 무관하게 뜬다):
...
> 툴이 노출되는 것과 **실행 가능**은 별개다 — 실제 활성 여부는 debug 툴을 호출해 `debug surface disabled`
> 거부가 나지 않는지로 확인한다.
